...
class FaceViewController: UIViewController
{
    ...
    private func updateUI() {
        if faceView != nil {
            ...
            faceView.navBarHeight = (self.navigationController?.navigationBar.bounds.height)!
        }
    }
    ...
    @IBOutlet var lineThicknessSlider: UISlider!
    
    @IBAction func changeThickness(sender: UISlider) {
        let sliderVal: Double = Double(lineThicknessSlider.value ?? 5.0)
        faceView.lineWidth = CGFloat(sliderVal + sliderVal)
    }
    
    //Want to do something here where I update the slider to be vertical
    override func willRotateToInterfaceOrientation(toInterfaceOrientation: UIInterfaceOrientation, duration: NSTimeInterval) {
        /*if toInterfaceOrientation.isLandscape {
            lineThicknessSlider.transform = CGAffineTransformMakeRotation(CGFloat(-M_PI/2))
            print("landscape")
        }
        else{
            lineThicknessSlider.transform = CGAffineTransformMakeRotation(CGFloat(0))
            print("vertical")
        }*/
        updateUI()
    }
